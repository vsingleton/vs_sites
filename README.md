...
tofu init
tofu apply -target=google_storage_bucket.tfstate -var-file=tim.tfvars

# 2. Uncomment the backend block in backend.tf, then migrate state:
tofu init -backend-config=backend.hcl

# 3. Apply everything else:
tofu apply -var-file=tim.tfvars
```

## Day-to-day usage

```bash
tofu plan -var-file=tim.tfvars
tofu apply -var-file=tim.tfvars
```

## Adding a subdomain

1. Add the label to `subdomains` in `tim.tfvars`
2. `tofu apply -var-file=tim.tfvars`
